...
declare variable $endAfterChanges as xs:integer := xs:integer($end);
...
declare function tour:main () as element() {
    ...
    if (not($endAfterChanges = 1 to 64))
    then
...
declare function tour:make-best-move (
    ...
    let $final-board as xs:integer* :=
        if ($move < $endAfterChanges) (:count($next-board[.=0])!=0:)
            ...
            as element()
{
           ...
           <tr>
              {for $column in 0 to 7
                ...
                <td align="center" bgcolor="{$color}" width="22">{
                  ...
                  return
                      if ($endAfterChanges != 64 and $n = $endAfterChanges)
                      then <b>{$n}</b>
                      else if ($n = 0)
                      then "&#xa0;"
                      else $n
    ...
    <p>{
        if ($endAfterChanges != 64)
        then
          <a href="Tour?start={$start}&amp;end={$endAfterChanges+1}">Step</a>
        else ()
    }</p>
    </div>
    </body>
